...
orca_grob <- rasterGrob(img, interpolate = TRUE)
...
ghg_emissions %>% select(Calendar_Year, Sector_General, Commodity_Type, Emissions_mtCO2e) %>% group_by(Calendar_Year, Sector_General) %>% summarise(mean_ghg = mean(Emissions_mtCO2e, na.rm = T), .groups = "drop")  %>%  
  ggplot(aes(x =factor(Calendar_Year), y = mean_ghg, fill = fct_reorder2(mean_ghg, factor(Calendar_Year)))) + 
  geom_col(aes(fill = Sector_General), position = "dodge") + 
  theme_bw() 
?fct_reorder
...
# could also do projected emissions with regression lines
